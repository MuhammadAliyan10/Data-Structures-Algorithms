class Stack:
    ...
    def popFromRight(self):
        if self.topOfRight < 100:
            self.stackArray[self.topOfRight] = 0
            self.topOfRight += 1
        else:
            print("Stack is empty from right.")
    def isEmptyFromLeft(self):
        True if self.topOfLeft == -1 else False
    def isEmptyFromRight(self):
        True if self.topOfRight == self.quantity - 1 else False


stack:object = Stack(10)
